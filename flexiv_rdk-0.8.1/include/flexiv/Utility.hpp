...
inline std::vector<double> m2mm(const std::vector<double>& mVec)
{
    std::vector<double> mmVec = {};
    for (const auto& v : mVec) {
        mmVec.push_back(v*1000);
    }
    return mmVec;
}

inline std::vector<double> slice(std::vector<double> const &v, int m, int n)
{
...
inline std::vector<double> mQuat2mmDeg(const std::vector<double>& mQuatVec)
{
    std::vector<double> mmDegVec;
    for (int i=0; i<3; i++) {
        mmDegVec.push_back(mQuatVec[i]*1000);
    }
    for (int i=0; i<3; i++) {
        mmDegVec.push_back(rad2Deg(quat2EulerZYX(slice(mQuatVec,3,6)))[i]);
    }
    return mmDegVec;
}

inline std::vector<double> ms2mms(const std::vector<double>& mQuatVec)
{
    std::vector<double> mmsVec;
    for (int i=0; i<3; i++) {
        mmsVec.push_back(mQuatVec[i]*1000);
    }
    for (int i=0; i<3; i++) {
        mmsVec.push_back((slice(mQuatVec,3,6))[i]);
    }
    return mmsVec;
}
